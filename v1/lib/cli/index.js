...
import { cmdAcceptProposal, cmdApplyAssistSignal, cmdAssistOpen, cmdAssistSignal, cmdDeclineProposal, cmdRepoAssistOpen, cmdTicketAssistOpen, cmdTicketStartRequest, formatProposal } from "./assist.js";
import { cmdDiagnose } from "./diagnose.js";
import { cmdCancelEpic, cmdFinalizeEpic, cmdStartEpic } from "./epic.js";
import { cmdLeaseAcquire, cmdLeaseGc, cmdLeaseList, cmdLeaseRelease } from "./lease.js";
import { cmdSubAgentContext } from "./sub-agent-context.js";
import { cmdGateSummary, cmdHumanDecision, cmdShowGate } from "./gate.js";
import { cmdCleanup, cmdClosePreflight, cmdCommitStep, cmdTicketClose, cmdTicketStart } from "./ticket.js";
import { cmdDoctor, cmdFlow, cmdFlowGraph, cmdLogs, cmdMetadata, cmdShowInterrupts, cmdStatus } from "./inspect.js";
import { runProviderCli } from "./provider-cli.js";
import { applyAssistSignalCommand, assertCurrentStep, assertRerunTarget, assertStepInVariant, assistOpenCommand, blockedStopCommands, firstLine, formatStepName, humanDecisionCommands, humanStopCommands, interruptAnswerCommands, interruptStopCommands, isHumanGateStep, nextProviderCommand, parseOptions, proposalCommands, proposalStopCommands, requireRuntime, rerunCurrentStepCommand, required, resumeCommand, runNextCommand, shellQuote, showGateCommand, sleep, stopCommand, statusCommand } from "./utils.js";
import { loadDotEnv } from "../support/env.js";
import { runtimeCliCommand } from "./cli-command.js";
import { describeFlow, buildFlowView, getInitialStep, getStep, loadFlow, nextStep, outcomeFromDecision, renderMermaidFlow } from "../flow/load.js";
import { evaluateStepGuards } from "../flow/guards/index.js";
import { runProvider, providerKillGraceMs } from "../runtime/providers/registry.js";
import { maybeFireDiagnoseWatchdog } from "../runtime/watchdog.js";
import { runCalcSmoke } from "../extras/smoke-calc.js";
import { archivePriorRunTag, commitStep, gateDecisionText, gateNoteHeadingsFor, gateTicketHeadingFor, stepCommitSummary, ticketClose, ticketCloseDryRun, ticketStart } from "../runtime/actions.js";
import { renderStepPrompt, writeStepPrompt } from "../flow/prompts/step.js";
import { writeReviewerPromptArtifact } from "../flow/prompts/reviewer.js";
import { writeReviewRepairPromptArtifact } from "../flow/prompts/repair.js";
import { captureNoteTicketPatchProposal, snapshotNoteTicketFiles } from "../extras/patch-proposals.js";
import { defaultAcceptedJudgementStatus, defaultJudgementKind, loadJudgements, writeJudgement } from "../flow/guards/judgement-artifact.js";
import { runFinalVerification } from "../flow/guards/final-verification.js";
import { formatDoctor, runDoctor } from "../runtime/doctor.js";
import { withRunLock } from "../runtime/locks.js";
import { acquireForTicket, releaseForTicket, LeaseConfigError, LeaseExhaustedError } from "../runtime/leases.js";
import { writeEnvLease, removeEnvLease } from "../runtime/env-lease.js";
import { answerLatestInterruption, createInterruption, latestOpenInterruption, loadStepInterruptions, renderInterruptionMarkdown } from "../runtime/interruptions.js";
import { writeFailureSummary } from "../runtime/failure-summary.js";
import { appendStepHistoryEntry, loadCurrentNote, replaceNoteSection, saveCurrentNote } from "../repo/note.js";
import { allowedAssistSignals, appendAssistSignal, appendTicketStartRequest, clearTicketStartRequest, loadLatestAssistSignal, markAssistSessionFinished, markAssistSessionStarted, prepareAssistSession, prepareTicketAssistSession, ticketAssistSessionPath, updateLatestAssistSignal } from "../runtime/assist/runtime.js";
import { resolveStepAgent } from "../runtime/providers/agent-resolution.js";
import { writeNoteOverrides } from "../repo/note-overrides.js";
import { activeReviewPlan, aggregateReviewerOutputs, expandReviewerInstances, loadReviewRepairOutput, loadReviewerOutput, loadReviewerOutputsForStepRound, recordAggregatorReviewArtifacts, reviewAccepted, reviewRoundDir, reviewRoundReviewerAttemptDir, reviewRoundReviewerOutputPath, reviewRepairOutputPath, writeLatestReviewerOutputMirror, writeReviewRepairResult, writeReviewRoundAggregate, writeReviewerAttemptResult } from "../runtime/review.js";
import { judgementFromUiOutput, loadStepUiOutput, uiOutputArtifactPath } from "../flow/prompts/ui-output.js";
import { writeStepUiRuntime } from "../runtime/ui.js";
import { clearStepCommitRecord, loadStepCommitRecord, writeStepCommitRecord } from "../runtime/step-commit.js";
import { appendProgressEvent, cleanupRunArtifacts, finishTrackedProcess, defaultStateDir, ensureCanonicalFiles, hasCompletedProviderAttempt, latestAttemptResult, latestHumanGate, clearHumanGateProposal, latestProviderSession, loadPdhMeta, loadRuntime, nextStepAttempt, recoverRuntimeFromTags, savePdhMeta, openHumanGate, progressPath, readProgressEvents, registerTrackedProcess, finishRunSupervisor, resetStepArtifacts, resolveHumanGate, saveRun, startRunSupervisor, startRun, stepDir, updateRun, updateRunSupervisor, updateHumanGateProposal, writeAttemptResult } from "../runtime/state.js";
const emitWarning = process.emitWarning.bind(process);
process.emitWarning = (warning, ...warningArgs) => {
    const warningName = typeof warning === "object" && warning !== null
        ? warning.name
...
try {
    ...
    else if (command === "start-epic") {
        await cmdStartEpic(args);
    }
...
Usage:
  ...
  pdh-flow start-epic --epic SLUG [--repo DIR] [--variant full|light] [--start-step PD-D-2] [--force-reset]
...
function validateTicketEpicLink({ mainRepo, ticketId }) {
    ...
    if (!epicSlugRaw || typeof epicSlugRaw !== "string")
        return;
    ...
    if (!existsSync(epicPath)) {
        throw new Error(`Ticket ${ticketId} declares epic="${epicSlug}" but epics/${epicSlug}.md does not exist on main. ` +
    ...
    if (!accepted.has(ticketBaseBranch)) {
        ...
        throw new Error(`Ticket/Epic branch mismatch: tickets/${ticketId}.md has base_branch="${ticketBaseBranch}" ` +
...
async function cmdResume(argv) {
    ...
    if (run?.id && isStaleSupervisor) {
        ...
        if (options["timeout-ms"])
            passthrough.push("--timeout-ms", options["timeout-ms"]);
        if (options["idle-timeout-ms"])
            passthrough.push("--idle-timeout-ms", options["idle-timeout-ms"]);
        if (options["max-attempts"])
            passthrough.push("--max-attempts", options["max-attempts"]);
        if (options["retry-backoff-ms"])
            passthrough.push("--retry-backoff-ms", options["retry-backoff-ms"]);
...
async function executeProviderStep({ repo, runtime, step: incomingStep, options }) {
    ...
    const agent = resolveStepAgent({ flow: runtime.flow, runtimeRun: runtime.run, step: incomingStep });
...
function maybeLockFlowVariant({ repo, runtime, step }) {
    if (runtime.run?.flow_variant_locked === true)
        return;
    try {
...
function syncFlowVariantFromNote({ repo, runtime, step }) {
    ...
    if (!noteVariant)
        return;
    if (!runtime.flow.variants?.[noteVariant])
        return;
    if (runtime.pdh?.variant === noteVariant)
        return;
...
function stripReviewerOutputForAggregator(output) {
    ...
    const slim = {};
    if (output.status)
        slim.status = output.status;
    if (output.summary)
        slim.summary = output.summary;
    if (Array.isArray(output.findings))
        slim.findings = output.findings;
...
async function runReviewRound({ repo, runtime, step, reviewPlan, reviewers, attempt, round, repairProvider, maxInPlaceRepairs, defaultRerunStep, inPlaceRepairCount, priorFindingsByReviewer, roundHistory, timeoutMs, idleTimeoutMs, options }) {
    const runId = runtime.run.id;
    const stepId = step.id;
    const stateDir = runtime.stateDir;
    // 1. Reviewers (parallel).
    const reviewerRuns = await Promise.all(reviewers.map((reviewer) => executeReviewerRun({
        repo,
        runtime,
        step,
        reviewer,
    ...
    if (!aggregatorUi || aggregatorUi.parseErrors?.length || !aggregatorJudgement || !aggregatorJudgement.status) {
        return {
            kind: "failed",
            lastResult: reviewRoundResult(4, `${stepId} aggregator did not produce a usable ui-output.json judgement in round ${round}`, aggregatorUi?.parseErrors?.join("\n") || "ui-output.json missing or judgement field absent"),
            rawLogPath: aggregatorRun.rawLogPath ?? null
        };
    }
    writeJudgement({
    ...
    if (!repair.output || repair.output.parseErrors?.length) {
        return {
            kind: "failed",
            lastResult: reviewRoundResult(4, `repair output missing or invalid in round ${round}`, repair.output?.parseErrors?.join("\n") || "repair output missing"),
            rawLogPath: repair.result.rawLogPath ?? null
        };
    }
...
function recordReviewBlockerEscalation({ repo, runId, stepId, attempt, round, targetStepId, aggregate, repair, blockers }) {
    ...
    lines.push("", "## ${targetStepId} 担当への申し送り".replace("${targetStepId}", targetStepId), "", "- 上記 blocker を解消する commit を作る (revert / 追加 commit / amend のどれかが必要)。", "- commit 後に runtime が再度 review を回します。", `- もとの ${stepId} round ${round} の repair artifact: \`.pdh-flow/runs/${runId}/steps/${stepId}/review-rounds/round-${round}/repair.json\``);
...
function finalizeEpicCompletedRun({ repo, runtime, step }) {
    ...
    if (!epicSlug) {
        throw new Error(`pdh-epic-core run ${runId} has no resolvable epic slug (ticket_id=${ticketLabel})`);
    }
    ...
    const finalizeRes = spawnSync(process.execPath, [cliPath, "finalize-epic", "--epic", epicSlug, "--repo", repo], { encoding: "utf8" });
    appendProgressEvent({
        repoPath: repo,
        runId,
        stepId: step.id,
    ...
    if (finalizeRes.status !== 0) {
        ...
        throw new Error(`pdh-flow finalize-epic failed for ${epicSlug}: ${(finalizeRes.stderr || finalizeRes.stdout || "unknown").trim()}`);
    }
    cleanupRunArtifacts({ repoPath: repo, runId });
    const pdh = loadPdhMeta(repo);
    savePdhMeta(repo, {
...
function summarizePartialReviewContext(reviewers) {
    ...
    const findings = reviewers.flatMap((reviewer) => (reviewer?.output?.findings ?? []).map((finding) => ({
        ...finding,
        reviewerId: reviewer.reviewerId,
...
function maybeStartTicket({ repo, ticket, worktree = false, required = false }) {
    if (!ticket) {
        return;
    }
    if (!existsSync(join(repo, "ticket.sh"))) {
