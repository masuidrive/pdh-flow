...
import { renderSubAgentContext, writeSubAgentContext } from "../flow/prompts/sub-agent.ts";
import { parseOptions } from "./utils.ts";

export function cmdSubAgentContext(argv: string[]) {
  ...
  const params = {
    ...
    outputSchema: outputSchema as "reviewer" | "repair" | "freeform",
