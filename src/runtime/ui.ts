...
import { defaultJudgementKind, loadJudgements } from "../flow/guards/judgement-artifact.ts";
import { loadStepInterruptions } from "./interruptions.ts";
import { getStep, loadFlow } from "../flow/load.ts";
import { extractSection } from "../repo/note.ts";
import { collectStepArtifacts, latestAttemptResult } from "./state.ts";
import {
  asRecordList,
  asString,
  asStringList,
  loadJsonArtifact,
  loadStepUiOutput
} from "../flow/prompts/ui-output.ts";
...
const STEP_HIGHLIGHT_FALLBACKS = {
  ...
  risks: {
    "PD-C-3": [{ anyOf: ["step:PD-C-2", "step:PD-C-4"], limit: 3 }],
    "PD-C-4": [{ anyOf: ["step:PD-C-2", "step:PD-C-4"], limit: 3 }],
    "PD-C-5": [{ anyOf: ["step:PD-C-2", "step:PD-C-4"], limit: 3 }],
    "PD-C-6": [{ anyOf: ["step:PD-C-6", "step:PD-C-2"], limit: 3 }],
