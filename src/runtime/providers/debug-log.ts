...
export type DebugLogContext = {
  ...
  stepId: string;
  roleId: string;
  artifactPaths?: { name: string; path: string }[];
};

export function readArtifactsForLog(artifactPaths: { name: string; path: string }[] = []) {
...
): { reqPath: string; seqStr: string } | null {
  if (!isProviderDebugLogEnabled()) return null;
  if (!ctx.stateDir || !ctx.runId || !ctx.stepId || !ctx.roleId) return null;
...
function nextSeqFor(logDir: string, stepId: string, roleId: string): number {
  if (!existsSync(logDir)) return 1;
