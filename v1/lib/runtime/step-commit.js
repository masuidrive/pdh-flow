...
import { CommandExecutionError, runCommandResult } from "../support/command.js";
...
function gitScalar(repoPath, args) {
    const result = runCommandResult("git", args, { cwd: repoPath });
    if (!result.ok) {
        process.stderr.write(`pdh-flow: warning: step-commit git query failed: ${new CommandExecutionError(result).message}\n`);
        return null;
    }
