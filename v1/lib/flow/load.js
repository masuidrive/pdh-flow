...
export function loadFlow(flowId = "pdh-ticket-core") {
    ...
    const top = (parse(readFileSync(path, "utf8")) ?? {});
    ...
    if (existsSync(stepsDir)) {
        for (const file of readdirSync(stepsDir).sort()) {
            if (!file.endsWith(".yaml"))
                continue;
            const stepDoc = (parse(readFileSync(join(stepsDir, file), "utf8")) ?? {});
            if (stepDoc?.id)
                steps.push(stepDoc);
        }
    }
...
// of new step objects.
export function hydrateFlowForVariant(flow, variant) {
    if (!flow)
        return flow;
    if (!variant) {
...
function flattenStepShape(step) {
    if (!step)
        return;
    const display = step.display ?? {};
    if (step.label === undefined && display.label !== undefined)
        step.label = display.label;
    if (step.summary === undefined && display.summary !== undefined)
        step.summary = display.summary;
    if (step.userAction === undefined && display.userAction !== undefined)
        step.userAction = display.userAction;
    ...
    if (step.mode === "review") {
        ...
        if (step.review.intent === undefined && prompt.intent !== undefined)
            step.review.intent = prompt.intent;
        if (step.review.passWhen === undefined && prompt.passWhen !== undefined)
            step.review.passWhen = prompt.passWhen;
        if (step.review.onFindings === undefined && prompt.onFindings !== undefined)
            step.review.onFindings = prompt.onFindings;
        if (step.review.reviewerRules === undefined && prompt.reviewerRules !== undefined)
            step.review.reviewerRules = prompt.reviewerRules;
        if (step.review.repairRules === undefined && prompt.repairRules !== undefined)
            step.review.repairRules = prompt.repairRules;
    }
    if (step.transitions) {
...
function validateStepReferences(flow) {
...
export function providerForStep(flow, step, variant) {
    if (!step)
        return null;
    if (!variant) {
...
function pickVariant(value, variant, where) {
    if (value == null)
        return null;
