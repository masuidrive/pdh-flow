...
import { createInterruption } from "../runtime/interruptions.ts";
import { withRunLock } from "../runtime/locks.ts";
import { appendProgressEvent, defaultStateDir, updateRun } from "../runtime/state.ts";
import { writeStepUiRuntime } from "../runtime/ui.ts";
import { assertCurrentStep, interruptStopCommands, parseOptions, requireRuntime } from "./utils.ts";
import type { CliOptions } from "../types.ts";

export async function runProviderCli(argv: string[]) {
...
async function cmdAsk(argv: string[]) {
...
function readMessageOption(options: CliOptions, commandName: string) {
