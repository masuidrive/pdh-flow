...
import { getStep } from "../flow/load.ts";
import { loadDotEnv } from "../support/env.ts";
import { latestHumanGate } from "../runtime/state.ts";
import { prepareAssistSession } from "../runtime/assist/runtime.ts";

import { cmdAssistSignal } from "./assist.ts";
import { withRuntimeLock } from "./index.ts";
import {
  assertCurrentStep,
  parseOptions,
  requireRuntime
...
export async function cmdDiagnose(argv) {
  ...
  await withRuntimeLock({
    ...
    action: async () => {
      ...
      if (!DIAGNOSABLE_STATUSES.has(runtimeStatus)) {
        throw new Error(
          `diagnose: run.status=${runtimeStatus} is not diagnosable (allowed: ${[...DIAGNOSABLE_STATUSES].join(", ")})`
        );
      }
  ...
  if (options.bare === "true") claudeArgs.push("--bare");
  ...
  if (!allowedSignals.includes(proposal.signal)) {
    throw new Error(
      `diagnose: claude emitted signal=${proposal.signal} but only ${allowedSignals.join(", ")} are allowed at this state`
    );
  }
...
function extractFinalAssistantText(streamJson) {
  ...
  if (assistantSegments.length > 0) return assistantSegments.join("\n");
  return resultText ?? "";
}
