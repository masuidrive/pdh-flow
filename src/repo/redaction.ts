...
export function createRedactor({ repoPath = process.cwd(), env = process.env }: { repoPath?: string; env?: NodeJS.ProcessEnv } = {}) {
  const exactSecrets = collectExactSecrets({ repoPath, env });
  return (value: unknown) => redactSecrets(value, { exactSecrets });
}

export function redactSecrets(value: unknown, { exactSecrets = [] }: { exactSecrets?: string[] } = {}) {
...
function collectExactSecrets({ repoPath, env }: { repoPath: string; env: NodeJS.ProcessEnv }) {
...
function addSecret(values: Set<string>, value: unknown) {
