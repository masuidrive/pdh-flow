...
advance_to_provider_step() {
  local repo="$1"
  local run_id
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant light --start-step PD-C-5 | sed -n '1p')"
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" >"$TMP_ROOT/$run_id.gate.json"
  node "$ROOT/src/cli/index.mjs" approve --repo "$repo" --step PD-C-5 --reason ok >/dev/null
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --stop-after-step >"$TMP_ROOT/$run_id.stop.txt"
  printf '%s\n' "$run_id"
}
...
test_frontmatter_run() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >"$TMP_ROOT/frontmatter.run.txt"
...
test_nested_section_guard() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >/dev/null
  node --input-type=module -e "import { replaceNoteSection } from '$ROOT/src/core/note-state.mjs'; replaceNoteSection('$repo', 'PD-C-3. 計画', '### 実装方針\\n\\n- nested plan detail\\n\\n### テスト計画\\n\\n- nested verification detail');"
  node --input-type=module -e "import { evaluateGuard } from '$ROOT/src/core/guards.mjs'; const result = evaluateGuard({ id: 'plan-recorded', type: 'note_section_updated', path: 'current-note.md', section: 'PD-C-3. 計画' }, { repoPath: '$repo' }); console.log(JSON.stringify(result));" >"$TMP_ROOT/nested-section-guard.json"
...
test_prompt_context() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >/dev/null
  prompt_path="$(node "$ROOT/src/cli/index.mjs" prompt --repo "$repo")"
  ...
  grep -q "node src/cli/provider-cli.mjs ask --repo . --message" "$prompt_path"
...
test_stop_after_step() {
  ...
  node "$ROOT/src/cli/index.mjs" status --repo "$repo" >"$TMP_ROOT/$run_id.status.txt"
...
test_blocked_run() {
  ...
  run_id="$(advance_to_provider_step "$repo")"
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --manual-provider >"$TMP_ROOT/$run_id.blocked.txt"
  grep -q "provider_step_requires_execution" "$TMP_ROOT/$run_id.blocked.txt"
  node "$ROOT/src/cli/index.mjs" status --repo "$repo" >"$TMP_ROOT/$run_id.blocked-status.txt"
...
test_auto_provider_run() {
  ...
  CODEX_BIN="$fake" FAKE_ARGS_FILE="$args" node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.auto-provider.txt" || true
  test -f "$args"
...
test_auto_review_judgement() {
  ...
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-4 | sed -n '1p')"
  ...
  CLAUDE_BIN="$fake_claude" CODEX_BIN="$fake_codex" FAKE_CLAUDE_ARGS_FILE="$args" \
    node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.review.txt"
...
test_review_loop_auto_repair() {
  ...
  CLAUDE_BIN="$fake_claude" CODEX_BIN="$fake_codex" FAKE_REVIEW_COUNT_FILE="$count_file" \
    node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.review-loop.txt"
...
test_review_guard_auto_repair() {
  ...
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-9 | sed -n '1p')"
  ...
  CLAUDE_BIN="$fake_claude" CODEX_BIN="$fake_codex" \
    node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.review-guard-repair.txt"
  ...
  node "$ROOT/src/cli/index.mjs" status --repo "$repo" >"$TMP_ROOT/$run_id.review-guard-status.txt"
...
test_failed_run() {
  ...
  CODEX_BIN="$fake" node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.provider.txt" || true
  ...
  node "$ROOT/src/cli/index.mjs" status --repo "$repo" >"$TMP_ROOT/$run_id.failed-status.txt"
...
test_auto_resume_after_idle_timeout() {
  ...
  CODEX_BIN="$fake" FAKE_ARGS_DIR="$args_dir" FAKE_COUNT_FILE="$count_file" \
    node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 2 --retry-backoff-ms 0 --timeout-ms 5000 --idle-timeout-ms 200 \
...
test_run_refuses_active_run() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >/dev/null
  if node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >"$TMP_ROOT/refuse.out" 2>"$TMP_ROOT/refuse.err"; then
...
test_force_reset_creates_archive_tag() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-3 >/dev/null
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-2 --force-reset >"$TMP_ROOT/force-reset.out"
...
test_step_recovery_tag() {
  ...
  (
    ...
    node "$ROOT/src/cli/index.mjs" commit-step --repo "$repo" --step PD-C-3 --message "Plan recorded" --ticket runtime-test >/dev/null
  )
...
test_pdh_stop_marks_user_stopped() {
  ...
  node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-2 >/dev/null
  node --input-type=module -e "import { defaultStateDir, startRunSupervisor } from '$ROOT/src/runtime/runtime-state.mjs'; startRunSupervisor({ stateDir: defaultStateDir('$repo'), repoPath: '$repo', runId: 'placeholder', stepId: 'PD-C-2', command: 'run-next', pid: process.pid });"
  node "$ROOT/src/cli/index.mjs" stop --repo "$repo" --reason "user requested" >"$TMP_ROOT/pdh-stop.json"
...
test_resume_after_process_lost() {
  ...
  run_id="$(advance_to_provider_step "$repo")"
  node --input-type=module -e "import { defaultStateDir, startRunSupervisor, loadRuntime } from '$ROOT/src/runtime/runtime-state.mjs'; const stateDir = defaultStateDir('$repo'); startRunSupervisor({ stateDir, repoPath: '$repo', runId: '$run_id', stepId: 'PD-C-6', command: 'run-next', pid: 999999 }); loadRuntime('$repo', { normalizeStaleRunning: true, staleAfterMs: 0 });"
  grep -q '"status": "failed"' "$repo/.pdh-flow/runtime.json"
  grep -q '"status": "stale"' "$repo/.pdh-flow/runtime-supervisor.json"
  local fake
  fake="$(write_fake_codex_success)"
  CODEX_BIN="$fake" node "$ROOT/src/cli/index.mjs" resume --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.resume-loss.txt" || true
  test -f "$repo/.pdh-flow/runs/$run_id/steps/PD-C-6/ui-output.json"
}
...
test_recover_from_tags_rebuilds_runtime() {
  ...
  (
    cd "$repo"
    git add current-note.md tickets/runtime-test-note.md 2>/dev/null || git add current-note.md
    node "$ROOT/src/cli/index.mjs" commit-step --repo "$repo" --step PD-C-3 --message "Plan" --ticket runtime-test >/dev/null
  )
  git -C "$repo" tag --list | grep -q "^pdh-flow/runtime-test/PD-C-3$"
  rm -rf "$repo/.pdh-flow"
  node "$ROOT/src/cli/index.mjs" recover --repo "$repo" >"$TMP_ROOT/recover.json"
...
test_resumed_run() {
  ...
  CODEX_BIN="$fake" FAKE_ARGS_FILE="$first_args" node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >/dev/null
  CODEX_BIN="$fake" FAKE_ARGS_FILE="$second_args" node "$ROOT/src/cli/index.mjs" resume --repo "$repo" --max-attempts 2 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.resume.txt"
...
test_interrupted_run() {
  ...
  node "$ROOT/src/cli/provider-cli.mjs" ask --repo "$repo" --message "Should multiplication use integer arithmetic?" >"$TMP_ROOT/$run_id.interrupt.txt"
  grep -q "interrupted" "$TMP_ROOT/$run_id.interrupt.txt"

  node "$ROOT/src/cli/index.mjs" status --repo "$repo" >"$TMP_ROOT/$run_id.interrupted-status.txt"
  grep -q "Status: interrupted" "$TMP_ROOT/$run_id.interrupted-status.txt"

  if CODEX_BIN="$fake" FAKE_ARGS_FILE="$args" node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.open-interrupt-provider.txt" 2>&1; then
  ...
  node "$ROOT/src/cli/index.mjs" answer --repo "$repo" --message "Yes. Preserve integer arithmetic for this fixture." >"$TMP_ROOT/$run_id.answer.txt"
  ...
  CODEX_BIN="$fake" FAKE_ARGS_FILE="$args" node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >"$TMP_ROOT/$run_id.answered-provider.txt"
...
test_assist_gate_flow() {
  ...
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-5 | sed -n '1p')"
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" >"$TMP_ROOT/$run_id.assist-gate-open.json"
  node "$ROOT/src/cli/index.mjs" assist-open --repo "$repo" --step PD-C-5 --prepare-only >"$TMP_ROOT/$run_id.assist-open.json"
  ...
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-5 --signal recommend-approve --reason ok --no-run-next >"$TMP_ROOT/$run_id.assist-signal.json"
  ...
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-5 --signal recommend-approve --reason ok --no-run-next >"$TMP_ROOT/$run_id.assist-signal-2.json"
  node "$ROOT/src/cli/index.mjs" accept-recommendation --repo "$repo" --step PD-C-5 --no-run-next >"$TMP_ROOT/$run_id.accept-recommendation.json"
...
test_gate_baseline_rerun_requirement() {
  ...
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-5 | sed -n '1p')"
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" >/dev/null
...
PY
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-5 --signal recommend-approve --reason "looks good" --no-run-next >/dev/null
  ...
  if node "$ROOT/src/cli/index.mjs" accept-recommendation --repo "$repo" --step PD-C-5 --no-run-next >"$TMP_ROOT/$run_id.accept-should-fail.txt" 2>&1; then
...
test_assist_rerun_recommendation() {
  ...
  run_id="$(node "$ROOT/src/cli/index.mjs" run --repo "$repo" --ticket runtime-test --no-ticket-start --variant full --start-step PD-C-5 | sed -n '1p')"
  node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" >/dev/null
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-5 --signal recommend-rerun-from --target-step PD-C-4 --reason "plan changed after discussion" --no-run-next >"$TMP_ROOT/$run_id.rerun-recommendation.json"
  grep -q '"target_step_id": "PD-C-4"' "$TMP_ROOT/$run_id.rerun-recommendation.json"
  node "$ROOT/src/cli/index.mjs" accept-recommendation --repo "$repo" --step PD-C-5 --no-run-next >"$TMP_ROOT/$run_id.accept-rerun.json"
...
test_assist_answer_flow() {
  ...
  run_id="$(advance_to_provider_step "$repo")"
  node "$ROOT/src/cli/index.mjs" interrupt --repo "$repo" --message "Need a decision on integer rounding." >/dev/null
  node "$ROOT/src/cli/index.mjs" assist-open --repo "$repo" --step PD-C-6 --prepare-only >"$TMP_ROOT/$run_id.assist-answer-open.json"
  ...
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-6 --signal answer --message "Keep integer arithmetic." --no-run-next >"$TMP_ROOT/$run_id.assist-answer.json"
...
test_assist_failed_continue() {
  ...
  CODEX_BIN="$fake_fail" node "$ROOT/src/cli/index.mjs" run-provider --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >/dev/null || true
  node "$ROOT/src/cli/index.mjs" assist-open --repo "$repo" --step PD-C-6 --prepare-only >"$TMP_ROOT/$run_id.assist-failed-open.json"
  ...
  node "$ROOT/src/cli/index.mjs" assist-signal --repo "$repo" --step PD-C-6 --signal continue --reason "edits are ready" --no-run-next >"$TMP_ROOT/$run_id.assist-failed-signal.json"
  ...
  node "$ROOT/src/cli/index.mjs" apply-assist-signal --repo "$repo" --step PD-C-6 --no-run-next >"$TMP_ROOT/$run_id.assist-failed-apply.json"
  ...
  CODEX_BIN="$fake_success" FAKE_ARGS_FILE="$args" node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --force --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >/dev/null || true
  test -f "$args"
}
...
test_web_readonly() {
  ...
  CODEX_BIN="$fake" FAKE_ARGS_FILE="$args" node "$ROOT/src/cli/index.mjs" run-next --repo "$repo" --max-attempts 1 --retry-backoff-ms 0 --timeout-ms 5000 >/dev/null || true
  server_log="$TMP_ROOT/web.log"
  node "$ROOT/src/cli/index.mjs" web --repo "$repo" --host 127.0.0.1 --port 0 >"$server_log" 2>&1 &
  server_pid="$!"
