...
import { commitStep, stepCommitSummary } from "./actions.js";
import { resolveStepReviewPlan } from "../flow/load.js";
import { defaultAcceptedJudgementStatus, defaultJudgementKind, writeJudgement } from "../flow/guards/judgement-artifact.js";
import { replaceNoteSection } from "../repo/note.js";
import { uiOutputArtifactPath } from "../flow/prompts/ui-output.js";
...
export function expandReviewerInstances(reviewPlan) {
    ...
    return reviewers.flatMap((reviewer) => {
        ...
        const providers = Array.isArray(reviewer.providers) && reviewer.providers.length
            ? reviewer.providers
...
export function aggregateReviewerOutputs({ step, reviewPlan, reviewers }) {
    ...
    const nonAccepted = acceptedStatus
        ...
        : null;
    const findings = reviewers.flatMap((reviewer) => (reviewer.output?.findings ?? []).map((finding) => ({ ...finding, reviewerId: reviewer.reviewerId, reviewerLabel: reviewer.label })));
...
export function loadReviewRepairOutput({ stateDir, runId, stepId, round }) {
    return loadNormalizedJson(reviewRepairOutputPath({ stateDir, runId, stepId, round }), normalizeReviewRepairOutput);
}
