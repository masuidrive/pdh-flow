...
import { acquireForTicket, gcLeases, LeaseConfigError, LeaseExhaustedError, listLeases, releaseForTicket } from "../runtime/leases.js";
import { writeEnvLease, removeEnvLease } from "../runtime/env-lease.js";
import { parseOptions, required } from "./utils.js";
...
export async function cmdLeaseAcquire(argv) {
...
export async function cmdLeaseRelease(argv) {
    const options = parseOptions(argv);
    const repo = resolve(options.repo ?? process.cwd());
    const ticketId = required(options, "ticket");
    const pools = collectMulti(argv, "--pool");
...
export async function cmdLeaseList(argv) {
    const options = parseOptions(argv);
    const repo = resolve(options.repo ?? process.cwd());
...
export async function cmdLeaseGc(argv) {
    const options = parseOptions(argv);
    const repo = resolve(options.repo ?? process.cwd());
