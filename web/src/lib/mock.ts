...
function makeStep(idx: number, currentIdx: number, currentStatus: ProgressStatus): StepView {
  ...
  return {
    ...
    summary: "",
    userAction: null,
    progress: { status, label: status, note },
...
function baseState(currentIdx: number, currentStatus: ProgressStatus, next: NextAction): AppState {
  ...
  return {
    ...
    runtime: {
      ...
      noteState: { ticket: "calc-multiply", current_step: def.id, status: "running" },
      currentStep: { id: def.id, label: def.label },
    ...
    current: { gate: null, interruptions: [], nextAction: next, stepArtifacts: [] },
    history: [],
    events: [],
    git: { branch: "epic/pdh-flowchart", head: "abcdef0", tickets: [] },
...
const MOCKS: Record<string, () => AppState> = {
  "pd-c-5-needs-human": () =>
    baseState(3, "needs_human", {
      ...
      actions: [
        { label: "Open Terminal", description: "Claude assist と相談しながら recommendation を作るか、必要な修正をここで進めます。", tone: "neutral", kind: "assist" },
