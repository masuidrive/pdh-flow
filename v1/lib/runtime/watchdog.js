...
import { runDir, loadRuntime, latestAttemptResult, latestHumanGate } from "./state.js";
...
export function isDiagnoseWatchdogEnabled({ options = {}, env = process.env } = {}) {
    if (options["auto-diagnose"] === "true")
        return true;
    if (options["auto-diagnose"] === "false")
        return false;
    if (env.PDH_AUTO_DIAGNOSE === "1")
        return true;
    if (env.PDH_AUTO_DIAGNOSE === "0")
        return false;
    return false;
}
...
export function recordDiagnoseAttempt({ stateDir, runId, stepId, contextKey, maxAttempts = DEFAULT_MAX_ATTEMPTS }) {
...
export function maybeFireDiagnoseWatchdog({ repo, options = {}, env = process.env, log = () => { } }) {
    if (!isDiagnoseWatchdogEnabled({ options, env }))
        return null;
    let runtime;
    try {
    ...
    if (!run?.id || !stepId)
        return null;
    if (!DIAGNOSABLE_STATUSES.has(run.status))
        return null;
...
function parseMaxAttempts(options = {}, env = process.env) {
