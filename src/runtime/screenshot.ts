...
async function decodeImage(srcPath: string): Promise<RawImage> {
...
async function resizeIfNeeded(image: RawImage, maxSide: number): Promise<RawImage> {
...
): Promise<{ srcBytes: number; destBytes: number; width: number; height: number }> {
...
export async function optimizeScreenshotsInDir(
  dir: string,
  options: { quality?: number; maxSide?: number; deleteOriginals?: boolean; logger?: (msg: string) => void } = {}
): Promise<{ converted: string[]; skipped: string[]; failed: Array<{ path: string; reason: string }> }> {
  const result: { converted: string[]; skipped: string[]; failed: Array<{ path: string; reason: string }> } = {
