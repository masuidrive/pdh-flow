...
import { clearTicketStartRequest, loadLatestAssistSignal, loadPendingTicketStartRequests } from "../runtime/assist/runtime.js";
import { createAssistTerminalManager } from "../runtime/assist/terminal.js";
import { evaluateAcVerificationTable } from "../flow/guards/ac-verification.js";
import { buildFlowView, getStep, loadFlow, nextStep, renderMermaidFlow, resolveStepReviewPlan } from "../flow/load.js";
import { expandReviewerInstances } from "../runtime/review.js";
import { renderStepPrompt } from "../flow/prompts/step.js";
import { renderReviewerPrompt } from "../flow/prompts/reviewer.js";
import { renderReviewRepairPrompt } from "../flow/prompts/repair.js";
import { loadStepInterruptions } from "../runtime/interruptions.js";
import { loadJudgements } from "../flow/guards/judgement-artifact.js";
import { extractSection, loadCurrentNote, parseStepHistory } from "../repo/note.js";
import { parseNoteOverrides, writeNoteOverrides } from "../repo/note-overrides.js";
import { CommandExecutionError, commandErrorDetails, formatCommandForDisplay, runCommandResult } from "../support/command.js";
import { createRedactor } from "../repo/redaction.js";
import { loadReviewerOutputsForStep } from "../runtime/review.js";
import { loadStepUiOutput } from "../flow/prompts/ui-output.js";
import { loadStepUiRuntime } from "../runtime/ui.js";
import { appendProgressEvent, hasCompletedProviderAttempt, latestAttemptResult, latestHumanGate, listTrackedProcesses, loadRuntime, readProgressEvents, runtimeMetaPath, stepDir } from "../runtime/state.js";
import { archivePriorRunTag, findTicketWorktreePath } from "../runtime/actions.js";
...
function updateNoteFrontmatterFromWeb({ repo, patch }) {
    ...
    if (sanitized.flow_variant !== undefined) {
        ...
        if (variantChanged) {
            const flow = loadFlow();
            const nextSequence = flow.variants?.[nextVariant]?.sequence ?? [];
            const allowedStepIds = new Set(nextSequence);
            const sourceOverrides = sanitized.agent_overrides
...
function runtimeLockHolder({ repo }) {
    try {
        const runtime = loadRuntime(repo, { normalizeStaleRunning: false });
        ...
        if (holder?.hostname && holder.hostname !== hostname())
            return holder; // foreign host — treat as held, can't probe
        ...
        try {
            ...
            if (err?.code === "EPERM")
                return holder; // alive, no permission to signal
...
function spawnRunNextIfClear({ repo, extraArgs = [] }) {
...
function collectState({ repo }) {
    ...
    const optionalDocs = loadOptionalRepoDocuments(repo, redactor, runtime);
...
function loadOptionalRepoDocuments(repo, redactor, runtime) {
    const result = {};
...
function resolveCurrentEpicSlug(repo, runtime) {
    const ticketLabel = runtime?.run?.ticket_id;
    if (typeof ticketLabel !== "string" || !ticketLabel)
        return null;
    ...
    const steps = view.steps.map((step, index) => {
        ...
        return {
            ...
            judgements: runtime.run?.id
                ...
                : [],
            reviewFindings: reviewerOutputs.flatMap((reviewer) => (reviewer.output?.findings ?? [])
...
function buildRuntimeNotice({ events, currentStepId }) {
    if (!currentStepId || !events?.length)
        return null;
    ...
    if (!latestNotice)
        return null;
    const failedGuards = latestNotice.payload?.failedGuards
...
function collectPromptPayload({ repo, stepId }) {
    ...
    if (!isReview && step.provider !== "runtime") {
        try {
            ...
            prompts.push({ kind: "step", label: "実行プロンプト", error: err.message });
        }
    }
    if (isReview) {
        ...
        try {
            ...
            prompts.push({ kind: "repair", label: "Repair (loop)", error: err.message });
        }
    }
    if (prompts.length === 0)
        return null;
...
function listEpics(repo) {
    ...
    if (existsSync(epicsDir)) {
        ...
        for (const filename of names) {
            ...
            if (epic)
                epicsBySlug.set(epic.slug, epic);
        }
    }
    ...
    if (existsSync(doneDir)) {
        ...
        for (const entry of entries) {
            ...
            if (epicsBySlug.has(slug))
                continue; // a stray active file wins
    ...
    for (const branchName of branchInfo.keys()) {
        ...
        for (const path of String(ls.stdout || "").split(/\r?\n/)) {
            ...
            if (!/^epics\/[^/]+\.md$/.test(path))
                continue;
            if (path === "epics/README.md")
                continue;
...
function collectEpicBranchInfo(repo) {
