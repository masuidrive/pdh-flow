...
import { evaluateAcVerificationTable } from "./ac-verification.js";
...
function checkStepCommitRecorded(guard, context = {}) {
...
function checkJudgementStatus(guard, context) {
    ...
    if (uiJudgement?.kind === guard.artifactKind) {
        ...
        if (parseErrors.length > 0) {
            return passIf(guard, false, `${guard.artifactKind} not materialized because ui-output.json has parse errors: ${parseErrors[0]}`);
        }
        return passIf(guard, false, `${guard.artifactKind} is present in ui-output.json (${uiJudgement.status}) but the judgement artifact was not written`);
    }
    if (context.latestAttempt?.status === "completed") {
        return passIf(guard, false, `${guard.artifactKind} is still missing even though the provider step completed; inspect ui-output.json and judgements/`);
    }
