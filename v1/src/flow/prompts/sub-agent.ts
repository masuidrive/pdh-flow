...
import { parseAcVerificationTable } from "../guards/ac-verification.ts";
import { getStep } from "../load.ts";
import { activeReviewPlan, expandReviewerInstances } from "../../runtime/review.ts";
import { loadRuntime, stepDir } from "../../runtime/state.ts";
...
type Options = {
  ...
  reviewerId?: string;
  files?: string[];
  outputSchema?: "reviewer" | "repair" | "freeform";
  priorStep?: string;
  timestamp?: string;
};
...
export function renderSubAgentContext(opts: Options): Rendered {
  ...
  let priorStep: { id: string; noteSectionMarkdown: string } | null = null;
  if (opts.priorStep) {
  ...
  let jsonShape: string | null = null;
...
export function writeSubAgentContext(opts: Options): Rendered {
...
function resolveReviewerInfo(runtime, stepId: string, reviewerId: string) {
...
function readOrPlaceholder(path: string, placeholder: string): string {
...
function extractAcVerificationSection(noteText: string, repoPath: string): string | null {
...
function extractStepNoteSection(noteText: string, stepId: string): string | null {
...
function extractMarkdownSection(text: string, headingNames: string[]): string | null {
