...
import { loadCurrentNote, updateCurrentNote } from "./note.js";
...
export function parseNoteOverrides(frontmatter) {
...
function parseAgentOverridesMap(input, warnings) {
    if (input == null)
        return {};
...
// dropped.
function parseStepOverride(stepId, raw, warnings) {
    if (raw == null)
        return null;
...
function parseEditAgent(raw, where, warnings) {
    if (raw == null)
        return null;
    if (typeof raw === "string") {
...
function parseReviewers(raw, where, warnings) {
    ...
    raw.forEach((item, i) => {
        ...
        if (typeof item.role === "string" && item.role)
            ...
            warnings.push(`${where}[${i}].role is required`);
