...
- Browser smoke (agent-browser, real claude, 2026-05-08): opened the turn-smoke flow's `ask_user` turn, the modal showed the cyan hint at the top of the rolling buffer, claude was at the chat prompt with auto-mode, sent a directive prompt asking claude to use the Bash tool to run `./.pdh-flow/bin/turn-respond --option 2`, claude exec'd the wrapper, engine's `awaitTurnAnswer` poller picked up `turn-001-answer.json`, engine resumed via its own short-lived `claude --resume`, produced the beret description, terminal reached. **Note**: claude defaults to staying in the engine's envelope-output system prompt (`{kind: 'final' | 'ask'}`) and won't go off-script unless told to. A simple "use the Bash tool to run X" override unblocks it.
