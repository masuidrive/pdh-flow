...
import { latestOpenInterruption } from "../interruptions.js";
import { latestAttemptResult, latestHumanGate } from "../state.js";
import { loadStepUiRuntime } from "../ui.js";
import { renderTemplate } from "../../flow/prompts/template-engine.js";
        ...
        const entries = readdirSync(dir, { withFileTypes: true })
            ...
            try {
                ...
                if (fm) {
                    ...
                    if (tm)
                        title = tm[1].trim().replace(/^["']|["']$/gu, "");
                    const bm = /^branch:\s*(.+)$/mu.exec(fm[1]);
                    if (bm)
                        branch = bm[1].trim().replace(/^["']|["']$/gu, "");
                }
            }
            catch {
